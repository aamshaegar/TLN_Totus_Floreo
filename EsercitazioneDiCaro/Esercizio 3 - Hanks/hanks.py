...
# resolve all pronouns: example = ["i", 'you', 'he', "she", "it", "we", "they"] => people
PRONOUNS = "-PRON-"
...
def disambiguate_sentence(context, subject, object_):
    ...
    if subject is not None:
        if subject[0] == PRONOUNS: synset_subj = SYNSET_PERSON
        else: synset_subj = lesk(context, subject[0], pos_mapping(subject[1]))

    if object_ is not None:
        if object_[0] == PRONOUNS: synset_obj = SYNSET_PERSON
